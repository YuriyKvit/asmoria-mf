...
class Server
{
    ...
    private $max_client = 5;
    private $host = 'asmoria.dev';
    private $port = 10001;
    private $workers = 1;
    private $pid;
    ...
    public function start()
    {
        ...
        $socket = stream_socket_server("tcp://{$this->host}:{$this->port}", $errorNumber, $errorString);
    ...
    protected function handshake($connect)
    {
        ...
        $info['Sec-WebSocket-Key'] = $match[1];
...
//        }
//        var_dump($data);exit;
        $message = 'пользователь #' . intval($connect) . ' (' . $this->pid . '): ' . strip_tags($data['payload']);

        $this->send($connect, $message);

        $this->sendHelper($connect, $message);
    }


    protected function onSend($connect, $data)
    {//вызывается при получении сообщения от мастера
        $this->sendHelper($connect, $data);
    }

    protected function send($connect, $message)
    {//отправляем сообщение на мастер, чтобы он разослал его на все воркеры
        echo fwrite($connect, $this->encode($message));
    }

    private function sendHelper($connect, $data)
    {
